...
namespace se {
  ...
  void SeDevice::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT &createInfo) {
    createInfo = {};
  ...
  void SeDevice::hasGflwRequiredInstanceExtensions() {
    ...
    for (const auto &extension : extensions) {
      spdlog::info('\t' + std::string(extension.extensionName));
    ...
    for (const auto &required : requiredExtensions) {
      spdlog::info('\t' + std::string(required));
      if (available.find(required) == available.end()) {
        spdlog::error("Missing required glfw extension: {}", required);
  ...
  void SeDevice::createBuffer(
    ...
    if (vkCreateBuffer(device_, &bufferInfo, nullptr, &buffer) != VK_SUCCESS) {
      spdlog::error("Failed to create vertrx buffer!");
